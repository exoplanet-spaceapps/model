...
def fit_isotonic(y_true, y_prob):
    m=IsotonicRegression(out_of_bounds='clip'); m.fit(y_prob, y_true); return m

def fit_platt(y_true, y_prob):
    m=LogisticRegression(solver='lbfgs'); m.fit(y_prob.reshape(-1,1), y_true); return m
...
def reliability_plot(y_true,y_prob,path):
    ...
    plt.figure(figsize=(4,4)); plt.plot([0,1],[0,1],'--'); plt.plot(mean_pred, fracs, marker='o'); plt.xlabel('Predicted'); plt.ylabel('Observed'); plt.tight_layout(); plt.savefig(path,dpi=140); plt.close()
...
def run_and_save(y_true,y_prob,out_dir='artifacts',method='isotonic'):
    ...
    y_cal=apply_calibrator(cal,y_prob); brier=brier_score_loss(y_true,y_cal)
    joblib.dump(cal, os.path.join(out_dir,'calibrator.joblib'))
    reliability_plot(y_true,y_cal, os.path.join(out_dir,'calibration_cnn.png'))
